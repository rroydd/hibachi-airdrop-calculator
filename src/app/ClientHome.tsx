...
${numberWithCommas.format(userPoints)} points · calculated with ${publicShareUrl.toString()}`;
  ...
  async function copyResultImage() {
    try {
      ...
      ctx.fillText("Airdrop Calculator", 105, 215);
      ctx.fillStyle = "rgba(255, 247, 237, 0.68)";
      ctx.font = "500 24px Arial";
      ctx.fillText("created by @brelgino", 105, 255);

      ctx.fillStyle = "#ffffff";
      ctx.font = `${finalAirdropUsd >= 100_000 ? "700 70px" : "700 76px"} Arial`;
      ctx.fillText(usd.format(finalAirdropUsd), 105, 350);
      ctx.fillStyle = "rgba(255, 247, 237, 0.72)";
      ctx.font = "500 26px Arial";
      ctx.fillText("Final estimated airdrop value", 105, 394);

      ctx.fillStyle = "rgba(255, 255, 255, 0.08)";
      ctx.roundRect(105, 438, 990, 105, 22);
      ctx.fill();
